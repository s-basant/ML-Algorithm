...
#pca = PCA(n_components=514)

#principalcomp_x  = pca.fit_transform(x)
#principalcomp_vx  = pca.fit_transform(vx)

#x  = pd.DataFrame(data = principalcomp_x)
#vx  = pd.DataFrame(data = principalcomp_vx)
...
#classifier = tree.DecisionTreeClassifier(criterion='gini', max_depth=40, max_features='auto', min_samples_leaf=1, min_samples_split=5, random_state=4)

# classifier = tree.DecisionTreeClassifier(criterion='entropy', max_depth=20, max_features='auto', min_samples_leaf=1, min_samples_split=2, random_state=4)
...
count =0
file = open('Output/ds1Test-dt.csv', 'w')
